console.clear();
console.log('Starting Terminal Vast with vast aura from Lonely Meddy...');
...
const sessionDir = process.env.WA_AUTH_DIR || path.join(__dirname, 'sessions');
...
async function loadSession() {
    // User sessions are created by WhatsApp-number pairing.
    // No SESSION_ID or remote credential download is required.
    return null;
}
...
async function clientstart(options = {}) {
    const webSession = Boolean(options.webSession);
    const pairingPhone = options.pairingPhone ? String(options.pairingPhone).replace(/\D/g, '') : null;
    const activeSessionDir = options.sessionDir || sessionDir;
    if (!fs.existsSync(activeSessionDir)) fs.mkdirSync(activeSessionDir, { recursive: true });

    console.log(chalk.cyan.bold(`[SESSION] Starting ${webSession ? 'web' : 'primary'} Terminal Vast session`));
    if (!webSession) {
...
*𝙶𝚛𝚘𝚞𝚙 : ${groupMetadata.subject}*

*You're the ${memberCount}th member!*
...
function writeSessionMeta(sessionPath, phone) {
    fs.mkdirSync(sessionPath, { recursive: true });
    fs.writeFileSync(path.join(sessionPath, 'terminal-vast.json'), JSON.stringify({ phone, createdAt: Date.now() }, null, 2));
}

function readSessionMeta(sessionPath) {
    try {
        return JSON.parse(fs.readFileSync(path.join(sessionPath, 'terminal-vast.json'), 'utf8'));
    } catch (_) {
        return null;
    }
}
...
async function restoreWebSessions() {
    const root = path.join(sessionDir, 'web');
    ...
    for (const entry of entries) {
        ...
        if (!meta?.phone || !fs.existsSync(creds)) continue;
        const id = entry.name;
        if (webSessions.has(id)) continue;
        const record = { status: 'reconnecting', phone: meta.phone, sessionDir: dir, startedAt: Date.now() };
        webSessions.set(id, record);
        try {
            const conn = await clientstart({ webSession: true, pairingPhone: meta.phone, sessionDir: dir, userId: id });
            webSessions.set(id, { ...record, conn, code: null });
            console.log(`[SESSION] Restored ${meta.phone}`);
        } catch (err) {
            webSessions.delete(id);
            console.error(`[SESSION] Restore failed for ${meta.phone}:`, err.message);
        }
    }
}
...
app.post('/api/pair', async (req, res) => {
    ...
    writeSessionMeta(sessionPath, phone);
    const entry = { status: 'pairing', phone, sessionDir: sessionPath, startedAt: now };
    webSessions.set(id, entry);

    try {
        const conn = await clientstart({
            webSession: true,
            pairingPhone: phone,
        ...
        return res.status(500).json({ error: 'Unable to create the WhatsApp pairing session.' });
    }
});

app.get('/api/session/:id', (req, res) => {
    const session = webSessions.get(req.params.id);
...
const server = app.listen(port, '0.0.0.0', (err) => {
    if (err) {
        ...
        console.log(`[Terminal Vast] HTTP server listening on 0.0.0.0:${port}`);
        if (process.env.PRIMARY_PHONE) {
            clientstart().catch(err => console.error('[PRIMARY] Startup failed:', err));
        } else {
            console.log('[PRIMARY] Disabled. Users connect using WhatsApp-number pairing at /api/pair.');
        }
        restoreWebSessions().catch(err => console.error('[SESSION] Restore startup failed:', err));
    }
});
